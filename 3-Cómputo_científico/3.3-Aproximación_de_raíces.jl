...
Si denominamos $x_1$ a la intersección de $g(x)$ con el eje x, tenemos: 
$$x_1 = x_0 - \frac{f(x0)}{f'(x0)}$$

Por lo que. generalizando esta expresión para el $x_{i+1}$, tenemos:
...
begin
	function newton(f, f′, x0, n)   #declaramos nuestra función
		aprox_raiz= x0 - (f(x0)/f′(x0)) #fórmula para calcular la aproximacion de la raiz

		for i in 1:n #establecemos el rango de 1 a n para que se repita el ciclo n veces con paso 1
			aprox_raiz = aprox_raiz - (f(aprox_raiz)/f′(aprox_raiz)) #renombramos la variable
		end
		print(aprox_raiz) #imprima el resultado
	end
end

# ╔═╡ 17fae714-0493-4fac-807a-269362eb251e
begin
	j(x)= x^3-x-1 #ejemplo de función
	j′(x)= 3*x^2  #derivada de la función  
end
...
# Tu código (comentado) va aquí :D
begin
	function newtonRecursiva(f, f′, x0, n) #declaramos la función
		aprox_raiz = x0-(f(x0)/f′(x0)) #fórmula para calcular la aproximacion de la raiz
		n-=1 #se vaya restando de uno en uno
		
		if n!=0 #que el ciclo se pare cuando n sea igual a cero
			newtonRecursiva(f, f′, aprox_raiz, n) #se haga la nueva función
		end
		print(" ")
		print(aprox_raiz)
	end
...
# ╔═╡ 9006e823-e30e-4b6a-8b2b-ad965ce6e9b4
newtonRecursiva(j,j′,1,6)
...
# ╔═╡ ac48e8e9-4bb0-43e3-b223-d6396a7dc987
begin
	function newtonDistancia(f,f′,x0,ϵ) #Declaramos la función
	if ϵ <= 0 #ponemos la condición de que epsilon debe ser mayor que cero
		print("Error, el valor de epsilon es menor o igual a cero")
	end
	x = x0-(f(x0)/f′(x0)) #declaramos la fórmula para calcular la aproximación de la raíz
		
		while  #mientras que el se cumpla que x-x0 es menor que ϵ, que me devuelva x
			abs(x-x0) < ϵ
		return x
		end
	end
...
# ╔═╡ 136f0332-f29d-40cb-8ef5-ef362e5ee5c7
newtonDistancia(j,j′,1, 4)
...
# ╔═╡ 01216ae1-254c-4ea1-95b0-757bc8924db1
begin  
	x = 1
	h = 10000
end

# ╔═╡ 546e045a-e189-416f-b137-78fc7a37df43
derivadaNumerica(j,x,h)
...
# ╔═╡ 53148e4c-5a54-4ae1-9091-7749c0772838
newtonDN(f′,x0,h) = x0 - (f′(x0))/derivadaNumerica(f′,x0, h)

# ╔═╡ 02b1535d-2e18-4761-94fa-0d7030dea5fb
newtonDN(j′,1,4)
...
# ╔═╡ b3b01771-c345-48ea-a2fd-80baaf45e8e2
@bind h1 Slider(1000:1000:1000000, default=0.5)

# ╔═╡ bbc45a35-4324-4e0a-aefa-64dca99fb62f
@bind x0 Slider(-10:0.1:10, default=0.5)

# ╔═╡ cf074857-ab1c-4547-acfd-046d8083f8f8
begin
F(x) = cos(x) #declaramos la función
df(x) = -sin(x) #derivada

	function newton(F, df, x0, eps=1e-7, max_iter=10) # Implementamos la ecuación del método de Newton, donde implementaremos lo de ejercicios anteriores
    x = x0 #renombramos
   		for _ in 1:max_iter 
        x_00 = x
        fx = F(x)
        dfx = df(x)
        	...
        	if abs(dfx) < eps #condiciones que cumple la función
            return x
        	end
		
        x = x_00 - fx/dfx #Ecuación del método de newton
	...
	function graficar(F, raices, limites=(-2π, 2π)) #graficar la función
    x = range(limites[1], limites[2], length = 1000)
    y = F.(x)

    plot(x, y, label="cos(x)", linewidth=3)
	scatter!(raices, F.(raices), label="Raices", color="green")
	end
raices = []

	for x0 in -2π:π/2:2π, h1 in 1000:1000:1000000
    raiz = newton(F, df, x0)
    push!(raices, raiz)
	end
...
# ╔═╡ 90a228ac-afd7-4b6d-98cb-82e20c9af426
newton(j,j′,1,3)
